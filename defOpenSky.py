def pull_opensky(planes):
    ...
    opens_api = OpenSkyApi(
        username=None
        if main_config.get("OPENSKY", "USERNAME").upper() == "NONE"
        else main_config.get("OPENSKY", "USERNAME"),
        password=None
        if main_config.get("OPENSKY", "PASSWORD").upper() == "NONE"
        else main_config.get("OPENSKY", "PASSWORD").upper(),
    )
    failed = False
    icao_array = []
