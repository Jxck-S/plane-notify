...
from ss import get_adsbx_screenshot
...
class Plane:
    def __init__(self, icao, config_path, config):
        ...
        if self.config.getboolean("PUSHBULLET", "ENABLE"):
            ...
            self.pb_channel = self.pb.get_channel(
                self.config.get("PUSHBULLET", "CHANNEL_TAG")
            )
    ...
    def run_opens(self, ac_dict):
        ...
        try:
            self.icao = ac_dict.icao24.upper()
            self.callsign = ac_dict.callsign
            self.latitude = ac_dict.latitude
            self.longitude = ac_dict.longitude
            self.on_ground = bool(ac_dict.on_ground)
            self.squawk = ac_dict.squawk
            self.track = ac_dict.heading

            if ac_dict.baro_altitude is not None:
    ...
    def run_adsbx_v1(self, ac_dict):
        ...
        try:
            ...
            self.icao = ac_dict["icao"].upper()
            self.callsign = ac_dict["call"]
            self.reg = ac_dict["reg"]
            self.latitude = float(ac_dict["lat"])
            self.longitude = float(ac_dict["lon"])
            self.alt_ft = int(ac_dict["alt"])
            self.on_ground = bool(int(ac_dict["gnd"]))
            self.squawk = ac_dict["sqk"]
            self.track = float(ac_dict["trak"])

            if self.on_ground:
                self.alt_ft = 0
            self.last_pos_datetime = datetime.fromtimestamp(
                int(ac_dict["postime"]) / 1000
            )
        except ValueError as e:
    ...
    def run_adsbx_v2(self, ac_dict):
        ...
        print(ac_dict)
        try:
            self.icao = ac_dict["hex"].upper()
            self.latitude = float(ac_dict["lat"])
            self.longitude = float(ac_dict["lon"])
            self.speed = ac_dict["gs"]
    ...
    def __str__(self):
        ...
        output = [
            ...
            [
                (Fore.CYAN + "Last Contact" + Style.RESET_ALL),
                (
                    Fore.LIGHTGREEN_EX
            ...
            [
                (Fore.CYAN + "On Ground" + Style.RESET_ALL),
                (Fore.LIGHTGREEN_EX + str(self.on_ground) + Style.RESET_ALL),
            ]
            if self.on_ground is not None
            else None,
            [
                (Fore.CYAN + "Baro Altitude" + Style.RESET_ALL),
                (
                    Fore.LIGHTGREEN_EX
                    + str("{:,} ft".format(self.alt_ft))
                    + Style.RESET_ALL
                ),
            ]
    ...
    def run_check(self):
        ...
        # Set status for landing plausible
            ...
            from airport_utils import getClosestAirport
        ...
        if self.landed or self.tookoff:
            ...
            print(
                ...
                state,
                "Municipality:",
                municipality + Style.RESET_ALL,
            )
        title_switch = {"reg": self.reg, "callsign": self.callsign, "icao": self.icao}
        # Set Discord Title
        if self.config.getboolean("DISCORD", "ENABLE"):
            self.dis_title = (
                (title_switch.get(self.config.get("DISCORD", "TITLE")) or "NA").strip()
                if self.config.get("DISCORD", "TITLE") in title_switch.keys()
                else self.config.get("DISCORD", "TITLE")
            )
        # Set Twitter Title
        if self.config.getboolean("TWITTER", "ENABLE"):
            self.twitter_title = (
                (title_switch.get(self.config.get("TWITTER", "TITLE")) or "NA")
                if self.config.get("TWITTER", "TITLE") in title_switch.keys()
                else self.config.get("TWITTER", "TITLE")
            )
        ...
        if self.tookoff or self.landed:
            ...
            if self.config.get("MAP", "OPTION") == "GOOGLESTATICMAP":
                get_map(
                    (municipality + ", " + state + ", " + country_code),
                    self.map_file_name,
                )
            elif self.config.get("MAP", "OPTION") == "ADSBX":
                url_params = (
                    f"icao={self.icao}&zoom=9&largeMode=2&hideButtons&hideSidebar&mapDim=0&overlays="
                    + self.get_adsbx_map_overlays()
                )
            ...
            if self.config.getboolean("DISCORD", "ENABLE"):
                ...
                send_discord_message(
                    dis_message, self.config, self.map_file_name, role_id=role_id
                )
            # PushBullet
            if self.config.getboolean("PUSHBULLET", "ENABLE"):
                with open(self.map_file_name, "rb") as pic:
                    map_data = self.pb.upload_file(
                        pic, "Tookoff IMG" if self.tookoff else "Landed IMG"
                    )
                self.pb_channel.push_note(
                    self.config.get("PUSHBULLET", "TITLE"), message
                )
            ...
            if self.config.getboolean("TWITTER", "ENABLE"):
                ...
                self.tweet_api.create_media_metadata(
                    media_id=twitter_media_map_obj.media_id, alt_text=alt_text
                )
        ...
        if (
            self.takeoff_time is not None
            and self.recheck_route_time is not None
            and (datetime.utcnow() - self.takeoff_time).total_seconds()
            ...
            if route_to is not None:
                ...
                if self.config.getboolean("DISCORD", "ENABLE"):
                    ...
                    send_discord_message(dis_message, self.config, role_id=role_id)
                # Twitter
                if self.config.getboolean("TWITTER", "ENABLE"):
                    # tweet = self.tweet_api.user_timeline(count = 1)[0]
                    self.latest_tweet_id = self.tweet_api.update_status(
                        status=f"{self.twitter_title} {route_to}".strip(),
        ...
        if self.feeding:
            ...
            if self.last_emergency is not None and not self.emergency_already_triggered:
                ...
                if (
                    ...
                    squawk_message = (
                        f"{self.dis_title} Squawking {self.last_emergency[1]} {emergency_squawks[self.squawk]}"
                    ).strip()
                    ...
                    if self.config.getboolean("DISCORD", "ENABLE"):
                        ...
                        send_discord_message(
                            dis_message, self.config, self.map_file_name
                        )
                    os.remove(self.map_file_name)
                    ...
                    if mode not in self.last_nav_modes:
                        ...
                        if self.config.getboolean("DISCORD", "ENABLE"):
                            ...
                            #    sendDis((dis_message + ", Sel Alt. " + str(self.sel_nav_alt) + ", Current Alt. " + str(self.alt_ft)), self.config)
                                ...
                                send_discord_message(dis_message, self.config)
            # Selected Altitude
            if (
                ...
                if (
                    ...
                    if distance_to_centroid <= 15:
                        ...
                        from_bearing = calculate_from_bearing(
                            (
                                float(nearest_airport_dict["latitude_deg"]),
                                float(nearest_airport_dict["longitude_deg"]),
                            ),
                        ...
                        if self.config.getboolean("DISCORD", "ENABLE"):
                            ...
                            send_discord_message(
                                message, self.config, self.map_file_name, role_id
                            )
                        if self.config.getboolean("TWITTER", "ENABLE"):
                            twitter_media_map_obj = self.tweet_api.media_upload(
                                self.map_file_name
                            )
    ...
    def check_new_ras(self, ras):
        for ra in ras:
            if (
                ...
                from ss import (
                    get_adsbx_screenshot,
                    generate_adsbx_screenshot_time_params,
                )
                ...
                if "threat_id_hex" in ra["acas_ra"].keys():
                    ...
                    url_params += (
                        generate_adsbx_screenshot_time_params(
                            ra["acas_ra"]["unix_timestamp"]
                        )
                        + f"&icao={ra['acas_ra']['threat_id_hex']},{self.icao.lower()}&timestamp={ra['acas_ra']['unix_timestamp']}"
                    )
                else:
