...
def get_adsbx_screenshot(
    ...
    try:
        element = browser.find_element(
            By.XPATH, "//*[contains(@id, 'FIOnDemandWrapper_')]"
        )
        browser.execute_script(
            """var element = arguments[0];    element.parentNode.removeChild(element); """,
            element,
    ...
    if conceal_pia or conceal_ac_id:
        blur_elements_by_id(
            browser, ["selected_callsign", "selected_icao", "selected_squawk1"]
        )
    if conceal_ac_id:
        blur_elements_by_id(
            browser,
    ...
    if "reg" in overrides.keys():
        ...
        browser.execute_script(
            f"arguments[0].innerText = '* {overrides['reg']}'", element
        )
        reg = overrides["reg"]
    else:
        try:
            reg = browser.find_element(By.ID, "selected_registration").get_attribute(
                "innerHTML"
            )
        ...
        try:
            try:
                ...
                if reg in photo_list.keys():
                    browser.execute_script(
                        "arguments[0].id = 'airplanePhoto';", photo_box
                    )
                    browser.execute_script(
                        "arguments[0].removeAttribute('width')", photo_box
                    )
                    browser.execute_script(
                        "arguments[0].style.width = '200px';", photo_box
                    )
                    browser.execute_script(
                        "arguments[0].style.float = 'left';", photo_box
                    )
                    browser.execute_script(
                        f"arguments[0].src = 'https://raw.githubusercontent.com/Jxck-S/aircraft-photos/main/images/{reg}.jpg';",
                        photo_box,
                    )
                    ...
                    if len(copy_right_children) > 0:
                        browser.execute_script(
                            f"arguments[0].innerText = 'Image © {photo_list[reg]['photographer']}'",
                            copy_right_children[0],
                        )
                    else:
                        browser.execute_script(
                            f"arguments[0].appendChild(document.createTextNode('Image © {photo_list[reg]['photographer']}'))",
                            image_copy_right,
                        )
        except Exception as e:
