...
class Plane:
    ...
    def __init__(self, icao, config_path, config):
        ...
        # Setup Tweepy
        if self.config.getboolean("TWITTER", "ENABLE") and Plane.main_config.getboolean(
            "TWITTER", "ENABLE"
        ):
            import tweepy

            twitter_app_auth = tweepy.OAuthHandler(
                Plane.main_config.get("TWITTER", "CONSUMER_KEY"),
                Plane.main_config.get("TWITTER", "CONSUMER_SECRET"),
            )
    ...
    def run_adsbx_v1(self, ac_dict):
        ...
        try:
            ...
            self.__dict__.update(
                {
                    "icao": ac_dict["icao"].upper(),
                    "callsign": ac_dict["call"],
                    "reg": ac_dict["reg"],
                    "latitude": float(ac_dict["lat"]),
                    "longitude": float(ac_dict["lon"]),
                    "alt_ft": int(ac_dict["alt"]),
                    "on_ground": bool(int(ac_dict["gnd"])),
                    "squawk": ac_dict["sqk"],
                    "track": float(ac_dict["trak"]),
                }
            )
            if self.on_ground:
                self.alt_ft = 0
        ...
        if self.tookoff or self.landed:
            ...
            if Plane.main_config.get("MAP", "OPTION") == "GOOGLESTATICMAP":
                ...
                get_adsbx_screenshot(
                    ...
                    conceal_ac_id=self.conceal_ac_id,
                    conceal_pia=self.conceal_pia,
                )
                from modify_image import append_airport

                text_credit = (
                    self.config.get("MAP", "TEXT_CREDIT")
            ...
            # Telegram
            if self.config.has_section("TELEGRAM") and self.config.getboolean(
                "TELEGRAM", "ENABLE"
            ):
            ...
            if self.config.getboolean("TWITTER", "ENABLE"):
                ...
                try:
                    ...
                    raise
            # Meta
            if self.config.has_option("META", "ENABLE") and self.config.getboolean(
                "META", "ENABLE"
            ):
            ...
            if self.landed:
                if (
                    ...
                    from_coord = (
                        nearest_from_airport["latitude_deg"],
                        nearest_from_airport["longitude_deg"],
                    )
                    to_coord = (
                        landed_airport["latitude_deg"],
                        landed_airport["longitude_deg"],
                    )
                    ...
                    if mode not in self.last_nav_modes:
                        ...
                        if self.config.getboolean("DISCORD", "ENABLE"):
                            ...
                            if mode == "Approach":
                                ...
                                sendDis(
                                    dis_message, self.config, None, self.map_file_name
                                )
                    ...
                    if distance_to_centroid <= 15:
                        ...
                        if Plane.main_config.getboolean("TFRS", "ENABLE"):
                                ...
                                if in_tfr is not None:
                                    ...
                                    for index, shape in enumerate(
                                        ...
                                        if polygon.contains(aircraft_location):
                                            in_tfr = {
                                                "info": tfr,
                                                "closest_shape_name": shape["txtName"],
                                            }
                                            break
                                        else:
                                            point_dists = []
                            ...
                            if in_tfr is not None:
                                for shape in in_tfr["info"]["details"]["shapes"]:
                                    if shape["txtName"] == in_tfr["closest_shape_name"]:
                                        valDistVerUpper, valDistVerLower = int(
                                            shape["valDistVerUpper"]
                            ...
                            for shape in shapes:
                                if shape["type"] == "poly":
                                    ...
                                    context.add_object(
                                        staticmaps.Marker(center1, color=staticmaps.RED)
                                    )
                            ...
                            def tfr_image(context, aircraft_coords):
                                ...
                                tfr_map_filename = (
                                    f"{tempfile.gettempdir()}/{self.icao}_TFR_.png"
                                )
                        ...
                        if self.config.getboolean("DISCORD", "ENABLE"):
                            ...
                            if tfr_map_filename is not None:
                                ...
                                sendDis(
                                    message, self.config, role_id, self.map_file_name
                                )
                        if self.config.getboolean("TWITTER", "ENABLE"):
                            twitter_media_map_obj = self.tweet_api.media_upload(
                                self.map_file_name
                            ...
                            media_ids = [twitter_media_map_obj.media_id]
                            if tfr_map_filename is not None:
                                twitter_media_tfr_map_obj = self.tweet_api.media_upload(
                                    tfr_map_filename
                                )
                            ...
                            self.tweet_api.update_status(
                                status=tweet, media_ids=media_ids
                            )
                        # Meta
