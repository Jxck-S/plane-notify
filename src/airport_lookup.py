...
def get_closest_airport(latitude, longitude, allowed_types):
    ...
    allowed_types_ses = ", ".join(["%s"] * len(allowed_types))
    sql = (
        "SELECT oaa.ident, oaa.type, oaa.name, oaa.lat, oaa.lon, oaa.elev as elevation_ft, "
        "oaa.continent, oaa.iso_country, oaa.iso_region, oaa.municipality, "
        "oaa.icao_code, oaa.iata_code, oaa.local_code, "
        "oar.name as region, "
        "oac.name as country, "
        ...
        "WHERE oaa.geog <> ST_MakePoint(%s, %s) AND type in ("
        + allowed_types_ses
        + ") "
