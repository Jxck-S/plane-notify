...
class Plane:
    ...
    def add_trace(self):
        if self.latitude and self.longitude and self.alt_ft is not None:
            ...
            if (last_trace and self.latitude != last_trace[1] and self.longitude != last_trace[2]) or not last_trace:
                trace = (time.time(), self.latitude, self.longitude, self.alt_ft, self.on_ground)
                self.traces.append(trace)
    def expire_traces(self):
        if self.traces:
            for trace in self.traces:
                trace_timestramp = trace[0]
                if (datetime.now() - datetime.fromtimestamp(trace_timestramp)).total_seconds() >= 30*60:
...
#Check if Landed
        ...
        if self.landed or self.tookoff:
            ...
            print (Fore.GREEN + "Country:", country, "State:", state, "City:", city + Style.RESET_ALL)
        #Title
        ...
        if self.tookoff or self.landed:
            ...
            if self.tookoff:
                ...
                if nearest_airport_dict is not None and self.nearest_from_airport is not None and nearest_airport_dict['icao'] != self.nearest_from_airport:
                    ...
                    second_message = f"{'{:,}'.format(round(distance_mi))} mile ({'{:,}'.format(round(distance_nm))} NM) flight from {nearest_from_airport['iata_code'] if nearest_from_airport['iata_code'] != '' else  nearest_from_airport['ident']} to {nearest_airport_dict['iata_code'] if nearest_airport_dict['iata_code'] != '' else nearest_airport_dict['ident']}"
                ...
                if abs(total_change) >= 720 and self.circle_history['triggered'] is False:
                    ...
                    if distance_to_centroid <= 15:
                        ...
                        if main_config.getboolean("TFRS", "ENABLE"):
                            ...
                            except (HTTPError, Timeout, ConnectionError, json.decoder.JSONDecodeError) as err:
                                    ...
                                    if in_tfr is not None:
                                        ...
                                        for index, shape in enumerate(tfr['details']['shapes']):
                                            ...
                                            if shape['type'] == "poly":
                                                ...
                                                points = get_circle_perimeter_coords(shape['lat'], shape['lon'], radius_km)
                                            elif shape['type'] in ["polyarc", "polyexclude", "linebuffer"]:
                                                points = shape['all_points']
                                            aircraft_location = Point(self.latitude, self.longitude)
                                            if polygon is None:
                                                if len(points) < 4:
                                                    raise ValueError("Less than 4 points occured on NOTAM ->", tfr['NOTAM'])
                        ...
                        def tfr_image(context, aircraft_coords):
                            ...
                            tfr_map_filename = f"{tempfile.gettempdir()}/{self.active_icao}_TFR_.png"
